...
/*
 * In case that the MQTT Server needs authentication is mandatory to add username and password fields to the function 
 * mqttConfiguration(char mqtt_server[], char client_id[], uint16_t mqtt_port, uint8_t security,char username[],char password[])
 * 
 * Ex: mqttConfiguration(HTTP_SERVER,"user", HTTP_PORT, 0, username, password)
 */

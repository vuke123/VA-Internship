/*
    --- NB_IoT_20 - FTPS UPLOAD - Uploading files to a FTPS server  ---

    Explanation: This example shows how to upload a file to a FTPS server
    from Waspmote
  ...
  if (error == 0)
  {
    ...
    if (error == 0)
    {
      ...
      if (error == 0)
      {
        ...
        ////////////////////////////////////////////////
        // 4.1. FTPS open session
        ////////////////////////////////////////////////
    
        error = BG96.ftpsOpenSession(ftp_server, ftp_port, ftp_user, ftp_pass);
    
        // check answer
        if (error == 0)
        {
          USB.println(F("4.1. FTPS open session OK"));
          ...
          if (error == 0)
          {
            ...
            USB.print(F("4.2. Uploading SD file to FTPS server done! "));
          ...
          if (error == 0)
          {
            USB.println(F("4.3. FTPS close session OK"));
          }
          else
          {
        ...
        {
          USB.print(F( "4.1. FTPS connection error: "));
          USB.println(error, DEC);          
        }
    }

/*  
 *  ------ GPRS_SIM928A_22 - Getting IMSI from SIM and IMEI -------- 
 *  
...
void setup()
{
    ...
    if ((answer == 1) || (answer == -3))
    {
        ...
        // 2. gets IMSI:
        while (GPRS_SIM928A.getIMSI() != 1);
        USB.print(F("IMSI: "));
        USB.println(GPRS_SIM928A.buffer_GPRS);

        // 3. gets IMEI:
        while (GPRS_SIM928A.getIMEI() != 1);
        USB.print(F("IMEI: "));
        USB.println(GPRS_SIM928A.buffer_GPRS);

        // 4. shows firmware:
        while (GPRS_SIM928A.firmware_version() != 1);
    ...
    // 6. powers off the GPRS_SIM928A module
    GPRS_SIM928A.OFF(); 
}

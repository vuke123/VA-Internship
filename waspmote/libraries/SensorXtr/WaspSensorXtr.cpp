...
#endif
	// In order to improve consumption, SPI multiplexer pins should be is LOW state
...
//******************************************************************************
// ATMOS14 / VP4 Sensor Class functions
//******************************************************************************

/*!
	\brief ATMOS 14 Class constructor
	\param socket selected socket for sensor
*/

ATMOS_14::ATMOS_14(uint8_t _socket)
{
	// store sensor location
	socket = _socket;
...
/*!
	...
	\return 1 if ok, 0 if something fails
*/
uint8_t ATMOS_14::read()
{
	SensorXtr.setMux(socket, ENABLED);

	// Initialize variables
	sensorATMOS14.vaporPressure = 0;
	sensorATMOS14.temperature = 0;
	sensorATMOS14.relativeHumidity = 0;
	sensorATMOS14.atmosphericPressure = 0;
	...
	while ((validMeasure == 0) && (retries < 3))
	{
		...
		response = sdi12Sensor.readMeasures(sensorNameStr, strlen(sensorNameStr),
											sensorATMOS14.sensorSerialNumber,
											sensorATMOS14.vaporPressure,
											sensorATMOS14.temperature,
											sensorATMOS14.relativeHumidity,
											sensorATMOS14.atmosphericPressure);
		...
		if ((sensorATMOS14.vaporPressure != 0)
		|| (sensorATMOS14.temperature != 0)
		|| (sensorATMOS14.relativeHumidity != 0)
		|| (sensorATMOS14.atmosphericPressure != 0))
		{
			validMeasure = 1;
			sensorATMOS14.relativeHumidity = sensorATMOS14.relativeHumidity * 100.0;
		}
		else
		{
			delay(1000);
...
/*!
	...
	\return 1 if ok, 0 if something fails
*/
uint8_t ATMOS_14::readSerialNumber()
{
	return read();
}
...
//******************************************************************************
// SO-421 Sensor Class functions
//******************************************************************************


/*!
	\brief SO421 Class constructor
	\param socket selected socket for sensor
*/
Apogee_SO421::Apogee_SO421(uint8_t _socket)
{
	// store sensor location
	socket = _socket;
...
/*!
	...
	\return 1 if ok, 0 if something fails
*/
uint8_t Apogee_SO421::read()
{
	SensorXtr.setMux(socket, ENABLED);

	// Initialize variables
	sensorSO421.calibratedOxygen = 0;
	sensorSO421.bodyTemperature = 0;
	sensorSO421.milliVolts = 0;
	float parameter4_dummy = -1000;
	memset(sensorSO421.sensorSerialNumber, 0x00, sizeof(sensorSO421.sensorSerialNumber));
	...
	{
		memset(sensorNameStr, 0x00, sizeof(sensorNameStr));
		strncpy(sensorNameStr, "SO421", 6);
		
		response = sdi12Sensor.readMeasures(sensorNameStr, strlen(sensorNameStr),
											sensorSO421.sensorSerialNumber,
											sensorSO421.calibratedOxygen,
											sensorSO421.milliVolts,
											sensorSO421.bodyTemperature,
											parameter4_dummy);
		
		if(response == 0)
		{
			memset(sensorNameStr, 0x00, sizeof(sensorNameStr));
			strncpy(sensorNameStr, "SO-421", 6);
		...
		if ((sensorSO421.calibratedOxygen != 0)
		|| (sensorSO421.bodyTemperature != 0)
		|| (sensorSO421.milliVolts != 0))
		{
			validMeasure = 1;
...
/*!
	...
	\return 1 if ok, 0 if something fails
*/
uint8_t Apogee_SO421::readSerialNumber()
{
	return read();
}
...
void weatherStation::ON()
{
	...
	waitFor("<END OF STARTUP MESSAGE>",10000);
	...
	USB.println(_buffer,_length);
		...
		strcpy((char*)_buffer, "GMX550");
	#endif

	if (strstr((char*)_buffer, "GMX100") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX101") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX200") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX240") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX300") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX301") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX400") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX500") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX501") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX531") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX541") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX550") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX551") != NULL)
	{
#if DEBUG_XTR > 1
	...
	else if (strstr((char*)_buffer, "GMX600") != NULL)
	{
#if DEBUG_XTR > 1
...
/*!
	...
	\return 1 if ok, 0 if something fails
*/
uint8_t weatherStation::enterCommand(char* str)
{
	//Enter setup mode
	uint8_t answer;
	Utils.setMuxAux2();
	
	/* Step 1: Enter to SETUP MODE */
	if (enterSetupMode() != 0)
	{
		return 1;
	}
...
/*!
	\brief Enter in setup mode
	\param void
	\return 1 if ok, 0 if something fails
*/
uint8_t weatherStation::enterSetupMode()
{
	uint8_t answer;
	
	// set delay to 20 ms after each "sendCommand" operation
	setDelay(20);
	delay(100);

	// enter new command
	sendCommand((char*)"\r", (char*)">", 1000);


	/* Step 1: Enter to SETUP MODE */
	char cmd_setup[] = "*\r";

	for (uint16_t i = 0; i < strlen(cmd_setup); i++)
	{
		sendCommand((uint8_t*)&cmd_setup[i], 1);
	}
	delay(100);
	answer = waitFor((char*)"\r\n>", 2000);
	setDelay(100);
	...
	{
#if DEBUG_XTR > 1
		USB.println("SETUP MODE error");
		USB.printf("Buffer: %s\r\n", (char*)_buffer);
#endif
		return 1;
	}
#if DEBUG_XTR > 1
		USB.println("SETUP MODE OK");
		USB.printf("Buffer: %s\r\n", (char*)_buffer);
#endif
	return 0;
}
...
/*!
	\brief Exit in setup mode
	\param void
	\return 1 if ok, 0 if something fails
*/
uint8_t weatherStation::exitSetupMode()
{
	uint8_t answer;
	...
	char cmd_exit[] = "EXIT\r";
	for (uint16_t i = 0; i < strlen(cmd_exit); i++)
	{
		sendCommand((uint8_t*)&cmd_exit[i], 1);
	}
	delay(100);
#if DEBUG_XTR > 1
	USB.printf("Buffer: %s\r\n", (char*)_buffer);
#endif
	answer = waitFor((char*)"EXIT", 2000);
	setDelay(100);

	if (answer == 0)
	{
#if DEBUG_XTR > 1
		USB.println("EXIT error");
		USB.printf("Buffer: %s\r\n", (char*)_buffer);
#endif
		return 1;
	}
...
uint8_t weatherStation::read()
{
	...
	answer = sendCommand((char*)CMD, STX, 2000);

	if (answer == 1)
	{
		//Wait for ETX (End of Text)
		answer = waitFor(ETX, 1000);

		if (answer == 1)
		{
			// check empty fields as ",," patterns
			char *pointer = strstr((char*)_buffer, ",,");
			uint32_t position = 0;

			while (pointer != NULL)
			{
				position = (uint32_t)pointer - (uint32_t)_buffer + 2;
				Utils.insert(_buffer, _bufferSize, '0', (uint16_t)position);
				_bufferSize++;

				pointer = strstr((char*)_buffer, ",,");
			}
	...
	//strcpy((char*)_buffer, "Q 044 000.02 013 145 001.45 145 015.15 131 0000 00000.080 000.080 Y 010 +52 +33 +1 2018-06-11T10:59:20.6 +12.0 0000");

	//GMX100 example frame
	//strcpy((char*)_buffer, "Q 00000.080 000.080 Y +26 -56 +1 2018-06-12T16:31:11.1 +12.0 00000");

	//GMX101 example frame
	//strcpy((char*)_buffer, "Q 0001 01.20 07:54 12:10 16:25 174:+18 17:03 17:45 18:24 +32 -17 +1 2017-01-20T11:46:01.3 +05.0 0000");

	//GMX501 example frame
	//strcpy((char*)_buffer, "Q 043 000.00 235 000 000.00 000 000.00 000 0100 0976.5 0976.5 0976.5 030 +024.4 +005.7 06.71 192 0001 00.00 1.1 +014.2 05:28 12:03 18:37 151:+40 19:11 19:53 20:37 +30 -58 +1 2019-04-05T10:38:31.2 +11.9 0000");

	//GMX531, GMX551 example frame
	//strcpy((char*)_buffer, "Q 264 000.05 052 000 000.00 000 000.00 000 0100 00000.000 000.000 N 148 1001.4 1001.4 1001.4 047 +025.2 +013.4 11.18 0006 00.00  +026 1.2 +017.9 06:49 11:47 16:46 153:+26 17:16 17:49 18:22 +03 -82 +1 2018-11-13T10:06:56.5 +11.8 0000");

	//GMX541
	//strcpy((char*)_buffer, "Q 264 000.05 052 000 000.00 000 000.00 000 0100 00000.000 000.000 N 148 1001.4 1001.4 1001.4 047 +025.2 +013.4 11.18 0006 00.00 0 0 1.2 +017.9 06:49 11:47 16:46 153:+26 17:16 17:49 18:22 +03 -82 +1 2018-11-13T10:06:56.5 +11.8 0000");

	//GMX550
	strcpy((char*)_buffer, "Q 118 000.00 351 000 000.00 000 000.00 000 0100 0992.0 0992.0 0992.0 041 +023.7 +009.9 08.94 00000.000 000.000 N 233 0 0 1.2 +015.6 06:37 11:44 16:50 142:+19 17:25 18:03 18:42 +00 +00 -1 2019-10-24T09:21:46.9 +11.8 0000");
	#endif
	...
	if (stationModel == WS_GMX100)
	{
		...
		sscanf ((char*)_buffer, buffer_table,
						&node_letter_local,
						precipTotal_local,
	...
	else if (stationModel == WS_GMX101)
	{
		//Frame is like:
	...
	else if (stationModel == WS_GMX200)
	{
		if (!gps)
		{
			//Frame is like:
	...
	else if (stationModel == WS_GMX240)
	{
		if (!gps)
		{
			//Frame is like:
	...
	else if (stationModel == WS_GMX300)
	{
		//Frame is like:
	...
	else if (stationModel == WS_GMX301)
	{
		//Frame is like:
	...
	else if (stationModel == WS_GMX400)
	{
		//Frame is like:
	...
	else if (stationModel == WS_GMX500)
	{
		if (!gps)
		{
			//Frame is like:
	...
	else if (stationModel == WS_GMX501)
	{
		if (!gps)
		{
			//Frame is like:
	...
	else if ((stationModel == WS_GMX531 || stationModel == WS_GMX541 || stationModel == WS_GMX551))
	{
		if (!gps)
		{
			//Frame is like:
	...
	else if (stationModel == WS_GMX550 || stationModel == WS_GMX600)
	{
		if (!gps)
		{
			//Frame is like:
...
//******************************************************************************
// SQ-100-X Sensor Class functions
//******************************************************************************

/*!
	\brief SQ100X Class constructor
	\param socket selected socket for sensor
*/
Apogee_SQ100X::Apogee_SQ100X(uint8_t _socket)
{
	// store sensor location
	socket = _socket;
...
/*!
	\brief reads the SQ100X sensor and converts its value into umol·m-2·s-1
	\param void
	\return 0 if invalid data. Measure otherwise
*/
float Apogee_SQ100X::read(void)
{
	// object to manage ADC
	LTC ltc;
	...
	// Conversion from voltage into umol·m-2·s-1
	radiation = radiationVoltage / 0.0001;
